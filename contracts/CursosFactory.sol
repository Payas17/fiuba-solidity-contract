...
contract CursosFactory is Ownable, CreditoToken {
    ...
    mapping(uint => uint8[]) public correlativasCurso;
    ...
    modifier tieneCorrelativas(address _alumno, uint _cursoId) {
        uint[] memory correlativas = new uint[](correlativasCurso[_cursoId].length);
        for(uint i=0; i < correlativasCurso[_cursoId].length; i++){
            uint correlativaId = correlativasCurso[_cursoId][i];
    ...
    function modificarCurso(uint _cursoId, string memory _nombre, uint8 _creditos, uint8[] memory _correlativas, bool _activo, address _profesor) public onlyOwner cursoExistente(_cursoId) {
        ...
        correlativasCurso[_cursoId] = _correlativas;
    }
    ...
    function aprobarAlumno(address _alumno, uint _cursoId, bool _soloCursada, uint8 _nota, uint _fechaDeAprobacion) public onlyProfessor(_cursoId) tieneCorrelativas( _alumno, _cursoId) materiaNoAprobada(_alumno, _cursoId){
        require(_nota > notaMinima || _nota < notaMaxima, "La nota no se encuentra entre las notas posibles`");
        aprobados[_alumno][_cursoId] = Aprobacion(_soloCursada, _nota, _fechaDeAprobacion);
        if (!_soloCursada) {
    ...
    function _removerAlumnoDelCurso(address _alumno, uint _cursoId) private materiaNoAprobada(_alumno, _cursoId) {
        delete aprobados[_alumno][_cursoId];
    }
    
    function removerAlumnoDelCurso(address _alumno, uint _cursoId) public onlyProfessor(_cursoId){
        _removerAlumnoDelCurso(_alumno, _cursoId);
    }
    
    function removerCursadasVencidas(address _alumno) public {
        for(uint i=0; i < cursos.length; i++){
            Aprobacion storage aprobacion = aprobados[_alumno][cursos[i].id];
            if (aprobacion.fechaDeAprobacion < block.timestamp - 548 days && aprobacion.soloCursada ){
                _removerAlumnoDelCurso(_alumno, cursos[i].id);
            }
        }
    }

    function getCurso(uint _cursoId) public view returns (uint, string memory, uint8, uint8[] memory, bool, address) {
        Curso memory curso = cursos[_cursoId];
        uint8[] memory correlativas = new uint8[](correlativasCurso[_cursoId].length);
        correlativas = correlativasCurso[_cursoId];
        return (curso.id, curso.nombre, curso.creditos, correlativas, curso.activo, curso.profesor);
    }
    
    function getAlumno(address _alumno) public view returns (address, uint,uint[] memory, bool[] memory, uint8[] memory, uint[] memory){
        bool[] memory soloCursadas = new bool[](_getCantidadAprobaciones(_alumno));
        uint8[] memory notas = new uint8[](_getCantidadAprobaciones(_alumno));
        uint[] memory fechas = new uint[](_getCantidadAprobaciones(_alumno));
        ...
        while(j < _getCantidadAprobaciones(_alumno)) {
            if (aprobados[_alumno][cursos[i].id].nota > 0){
                ...
                notas[j] = aprobados[_alumno][cursos[i].id].nota;
                fechas[j] = aprobados[_alumno][cursos[i].id].fechaDeAprobacion;
                soloCursadas[j] = aprobados[_alumno][cursos[i].id].soloCursada;
                j++;
            }
            i++;
        }
        return (_alumno, creditos[_alumno], cursosId, soloCursadas, notas, fechas);
    }
